...
module savestate (
  ...
  // the load on a drop
  reg [35:0] stage_buf = 0;
  reg stage_full_r = 0;
  reg stage_rdreq = 0;

  wire [35:0] stage_q = stage_buf;
  wire do_pop = stage_rdreq && stage_full_r;
  wire do_push = stage_wr && (~stage_full_r || do_pop);
  wire stage_empty = ~stage_full_r;
  wire stage_full = stage_full_r && ~do_pop;
  wire stage_pending = ~stage_empty;

  always @(posedge clk_mem) begin
    if (do_push) stage_buf <= {stage_addr, stage_data};
    if (do_push && ~do_pop) stage_full_r <= 1;
    else if (do_pop && ~do_push) stage_full_r <= 0;
  end
