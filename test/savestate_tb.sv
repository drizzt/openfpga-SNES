...
// latency behind READ_MEM_CLOCK_DELAY), the stage register, and read/write
// ordering.
//
...
module savestate_tb #(
  ...
  initial begin
    ...
    // ------------------------------------------------------------------
    // LOAD staging at loader pace (16 clk_mem/word): no word may be dropped
    err_snap = errors;
    ctrl_idle <= 0;  // a load in flight, so stage_lost latches
    ...
    k = 0;
    while (dut.stage_pending && k < 40000) begin
      @(posedge clk_mem);
      k = k + 1;
    end
    if (dut.stage_pending) begin
      $display("FAIL: stage never drained");
      errors = errors + 1;
    end
    ...
    if (stage_lost) begin
      $display("FAIL: stage dropped a word at loader pace");
      errors = errors + 1;
    end
    if (errors == err_snap) $display("PASS: staging drains at loader pace");

    // ------------------------------------------------------------------
    // Ordering: an engine read right behind a staged write sees the new word
    err_snap = errors;
    for (i = 0; i < 4; i = i + 1) begin
      @(posedge clk_mem);
      stage_addr <= (20'd8 + i[19:0]) * 2;
      stage_data <= 16'h7E3C | i[15:0];
      stage_wr   <= 1;
      @(posedge clk_mem);
      stage_wr <= 0;
      repeat (14) @(posedge clk_mem);
    ...
    if (errors == err_snap) $display("PASS: engine read sees the staged words");

    // ------------------------------------------------------------------
    // Overflow: back-to-back words beat the drain, stage_lost latches until idle
    err_snap = errors;
    for (i = 0; i < 2; i = i + 1) begin
      @(posedge clk_mem);
      stage_addr <= (20'd200 + i[19:0]) * 2;
      stage_data <= 16'hDEAD;
      stage_wr   <= 1;
    end
    @(posedge clk_mem);
    ...
    if (!stage_lost) begin
      $display("FAIL: back-to-back words did not raise stage_lost");
      errors = errors + 1;
    end
    ctrl_idle <= 1;
    repeat (8) @(posedge clk_mem);
    if (stage_lost) begin
      $display("FAIL: stage_lost did not clear at idle");
      errors = errors + 1;
    end
    if (errors == err_snap) $display("PASS: overflow raises stage_lost, idle clears it");

    // ------------------------------------------------------------------
    // LOAD stream: header read, SSADDR re-read, DMA-paced stream; hits must
    // ack inside the no-wait budget (~8 clk_sys, one DMA byte gap)
    ss_busy = 1;
    worst_hit_lat = 0;
